...
# Если элемент массива больше нуля - пушим в новый массив первый элемент исходного массива.
  rand_arr_modify << rand_arr[0] if v > 0
...
rand_arr_consecutive.each_with_index do |v, i|
  ...
  if rand_arr_consecutive.min == v
    # Если индексы стоят рядом плюсуем счётчик
    if rand_arr_consecutive[i] == rand_arr_consecutive[i+1]
      count_consecutive += 1
    end
  end
end
...
rand_arr_consecutive.each_with_index do |v, e|
  if rand_arr_consecutive.max == v
    if rand_arr_consecutive[e] == rand_arr_consecutive[e+1]
      count_consecutive += 1
    end
  end
end
